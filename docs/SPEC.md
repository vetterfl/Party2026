...
| **Frontend assets** | All self-hosted | No CDN. Fonts, CSS, JS all served by the app |
| **CSS** | Hand-crafted | No Bootstrap CDN — write custom CSS matching Midnight Pool design |
| **Fonts** | Downloaded + embedded | e.g. Space Grotesk woff2 files in `assets/fonts/` |
| **JS** | Vanilla, bundled | No external scripts. Minimal JS for language toggle + RSVP form interactivity |
...
All static files served by Echo's static middleware from `assets/`:
...
├── css/
│   ├── main.css       ← guest-facing (Midnight Pool design)
│   └── admin.css      ← admin panel (functional, minimal)
└── js/
    └── main.js        ← lang toggle, RSVP form JS, spell prefill
```

`@font-face` declarations in `main.css` reference `/assets/fonts/`. No Google Fonts, no external requests.
...
- Display / name: Space Grotesk (self-hosted woff2), light/bold weights, very large
- Body: Space Grotesk Regular or system-ui fallback
- No Google Fonts import — font files in `assets/fonts/`
...
Single admin user. Credentials in `site_config` (`admin_user`, `admin_password_hash` bcrypt). No user table.
...
## 16. Design Implementation Notes

Frontend design built using the `frontend-design` skill — produces distinctive, production-grade interfaces, avoiding generic AI aesthetics. Invoked when implementing each template.

All output must be self-contained: no CDN links, no external font imports, no remote scripts in any template.
